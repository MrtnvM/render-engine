...
class RenderableButton: UIButton, @MainActor Renderable {
    ...
    private func applyTitleStyle() {
        ...
        if let titleStyleDict = component.properties.getDictionary(forKey: "titleStyle") {
            ...
            if let _ = titleStyle.textTransform, let currentTitle = title(for: .normal) {
    ...
    private func setupImageContent() {
        ...
        Task {
            let image = await self.imageLoader.loadImage(
                from: imageSource,
